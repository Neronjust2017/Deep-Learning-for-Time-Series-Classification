...
def read_dataset(root_dir,archive_name,dataset_name):
    ...
    #     file_name = root_dir+'/archives/'+archive_name+'/Multivariate/'+dataset_name+'/'+dataset_name
    #
    #     x=arff.loadarff(file_name+'_TRAIN.arff')
    #     y=arff.loadarff(file_name+'_TEST.arff')
    #     print('TRAIN:')
    #     print(x)
    #     print('TEST:')
    #     print(y)
    if archive_name == 'Multivariate':
        file_name = root_dir+'/archives/MTS'+archive_name+'/'+dataset_name+'/'+dataset_name
        ...
        file_name = root_dir+'/archives/TSC/'+archive_name+'/'+dataset_name+'/'+dataset_name
        x_train, y_train = readucr(file_name+'_TRAIN.txt')
        x_test, y_test = readucr(file_name+'_TEST.txt')
        datasets_dict[dataset_name] = (x_train.copy(),y_train.copy(),x_test.copy(),
            y_test.copy())
    elif archive_name == 'myMTS':
        file_name = root_dir + '/archives/MTS/' + archive_name+'/'
        datas = np.ndarray(shape=(600,5000,12),)
        datas_label=np.ndarray(shape=(600,))
        for l in range(0,600):
            file = sio.loadmat(file_name+'TRAIN'+str(101+l)+'.mat')
            print(file)
            data= file.get('data')
            data= data.transpose()
            print(data)
        ...
        print(datas_label)
        x_train_1 = datas[:180]
        x_test_1 = datas[180:300]
        x_train_2 = datas[300:480]
        x_test_2 = datas[480:600]
        y_train_1 =datas_label[:180]
        y_test_1 = datas_label[180:300]
        y_train_2 =datas_label[300:480]
        y_test_2 = datas_label[480:600]
        ...
        datasets_dict[dataset_name] = (shuffled_x_train.copy(), shuffled_y_train.copy(), shuffled_x_test.copy(),
                                   shuffled_y_test.copy())

    elif archive_name=="TianchiECG":
        # root1 = '/home/lixiaoyu/project/tianchi/ECG/ECG_tianchi/data'
        root = 'data/archives/MTS/ECG_tianchi/data'
        ...
        train_ecg_features = os.path.join(root, 'train_ecg_features.pth')

        dd = torch.load(train_data)
        # cc =torch.load(config.train_ecg_features)
        ...
        sum3 = 0
        sum4 = 0
        label1 = [0, 4]  # 窦性心律 正常ECG （1）
        label2 = [7]  # 窦性心动过速 （2）
        label3 = [2]  # 都姓心率过慢 （3）
        label4 = [13]  # 心房颤动 （4）
        data1 = np.zeros(shape=(10000, 5000, 8))
        data2 = np.zeros(shape=(10000, 5000, 8))
        data3 = np.zeros(shape=(10000, 5000, 8))
        data4 = np.zeros(shape=(10000, 5000, 8))
        # label= file2idx['5.txt']
        # print(operator.eq(label,label1) )
        # for i in range(len(file2idx)):
        #     label= file2idx[i]

        for (k, v) in file2idx.items():
            file_path = os.path.join(train_dir, k)
            if operator.eq(label1, v) == True:
                file = pd.read_csv(file_path, sep=' ').values
                data1[sum1, :] = file
                sum1 = sum1 + 1
            elif v[0] == label2[0]:
                file = pd.read_csv(file_path, sep=' ').values
        ...
        train_data = np.ndarray(shape=(6000, 5000, 8))
        test_data = np.ndarray(shape=(2000, 5000, 8))
        train_label = np.ndarray(shape=(6000,))
        test_label = np.ndarray(shape=(2000,))

        train_data[:1500] = data1[:1500]
        train_data[1500:3000] = data2[:1500]
        train_data[3000:4500] = data3[:1500]
        train_data[4500:5500] = data4[:1000]
        train_data[5500:6000] = data4[:500]

        test_data[:500] = data1[1500:2000]
        test_data[500:1000] = data2[1500:2000]
        test_data[1000:1500] = data3[1500:2000]
        test_data[1500:2000] = data4[250:750]

        train_label[:1500] = 1
        train_label[1500:3000] = 2
        train_label[3000:4500] = 3
        train_label[4500:6000] = 4

        test_label[:500] = 1
        test_label[500:1000] = 2
        test_label[1000:1500] = 3
        test_label[1500:2000] = 4

        permutation = np.random.permutation(train_label.shape[0])
        shuffled_x_train = train_data[permutation, :, :]
        shuffled_y_train = train_label[permutation]

        permutation = np.random.permutation(test_label.shape[0])
        shuffled_x_test = test_data[permutation, :, :]
        shuffled_y_test = test_label[permutation]

        datasets_dict[dataset_name] = (shuffled_x_train.copy(), shuffled_y_train.copy(), shuffled_x_test.copy(),
                                       shuffled_y_test.copy())

    print("train:")
    print(datasets_dict[dataset_name][0])
    print("train label")
    print(datasets_dict[dataset_name][1])
    print("test")
    print(datasets_dict[dataset_name][2])
    print("test label")
    print(datasets_dict[dataset_name][3])
